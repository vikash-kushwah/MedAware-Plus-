...
<body style="background-image:url(images/bookback.jpg)">
	<div class="header">
		<ul>
			<li style="float:left;border-right:none"><a href="ulogin.php" class="logo"><img src="images/cal.png" width="30px" height="30px"><strong> MedAware plus+ </strong>Appointment Booking System</a></li>
	...
	<div class="sucontainer" style="background-image:url(images/bookback.jpg)">
		...
		<select name="city" id="states-list" class="demoInputBox"  onChange="getcity(this.value);" style="width:100%;height:35px;border-radius:9px">
		<option value="">Select states </option>
		<?php
		$sql1="SELECT distinct(states) FROM clinic";
         $results=$conn->query($sql1); 
		while($rs=$results->fetch_assoc()) { 
		?>
		<option value="<?php echo $rs["states"]; ?>"><?php echo $rs["states"]; ?></option>
		<?php
		}
		?>
		</select>
		...
		<input type="int" placeholder="Enter PinCode" name="fpin_code" required><br>
...
if(isset($_POST['submit']))
{
        ...
        $fpin_code=$_POST['fpin_code'];
		$cid=$_POST['Clinic'];
		$did=$_POST['Doctor'];
		$dov=$_POST['dov'];
		$status="Booking Registered.Wait for the update";
		$timestamp=date('Y-m-d H:i:s');
		$sql = "INSERT INTO book (Username,Fname,Gender,CID,DID,DOV,Timestamp,Status,pin_code) VALUES ('$username','$fname','$gender','$cid','$did','$dov','$timestamp','$status','$fpin_code') ";
		if(!empty($_POST['fname'])&&!empty($_POST['gender'])&&!empty($_SESSION['username'])&&!empty($_POST['Clinic'])&&!empty($_POST['Doctor']) && !empty($_POST['dov']) && !empty($_POST['fpin_code']))
		{
